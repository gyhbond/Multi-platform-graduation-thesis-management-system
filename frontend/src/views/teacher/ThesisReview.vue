<template>
  <div class="thesis-review-container">
    ...
    <el-dialog
      ...
      :title="currentThesis?.status === 'reviewed' ? '修改评阅' : '论文评阅'"
      width="500px"
    >
    ...
    <el-dialog
      ...
      width="90%"
      :fullscreen="true"
      :before-close="handleCloseAnnotation"
      class="thesis-annotation-dialog"
    >
...
const handleDownload = async (thesis) => {
  try {
    ...
    const downloadUrl = `${baseUrl}/api/thesis/download/${thesis.id}`
    ...
    try {
      ...
      const handle = await window.showSaveFilePicker({
        suggestedName: `${thesis.student.name}-thesis.${ext}`,
        types: [fileTypes[ext] || {
          description: '所有文件',
          accept: {
            '*/*': [`.${ext}`]
    ...
    if (error.name === 'TypeError' && !window.showSaveFilePicker) {
      try {
        ...
        link.href = url
        link.download = `${thesis.student.name}-thesis.${ext}`
